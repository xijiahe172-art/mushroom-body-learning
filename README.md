...
tables, is in [`PHASE5_BENCHMARK.md`](PHASE5_BENCHMARK.md), and the five-phase
summary with the recommendation is in [`FINAL_REPORT.md`](FINAL_REPORT.md). Two
...
| File | Contents |
| --- | --- |
| [`MODULE.md`](MODULE.md) | The module's own reference: schema, retrieval and value formulas, the injected format, and the anti-cheating rules. |
| [`FINAL_REPORT.md`](FINAL_REPORT.md) | Five-phase summary: per-phase test results, benchmark data, and the recommendation on enabling `active`. |
| [`PHASE3_ACCEPTANCE.md`](PHASE3_ACCEPTANCE.md) | What injection actually looked like in ten real active-mode sessions. |
| [`PHASE4_ACCEPTANCE.md`](PHASE4_ACCEPTANCE.md) | The anti-cheating layer, its acceptance criteria, and the defects found while building it. |
| [`PHASE5_BENCHMARK.md`](PHASE5_BENCHMARK.md) | The 64-run benchmark: six metrics per arm, the noise floor, and per-task tables. |
...
This started from watching a coding agent make the same mistake twice: same
failure, same wrong move, no memory of having been there before. That led to the
mushroom body — the structure in the fruit-fly brain where odour and outcome get
associated, which is a real, well-studied example of learning without a world
model. The goal was never to simulate those neurons. It was to give an agent the
smallest useful version of what they buy a fly: a tendency, formed from its own
experience, about what is worth trying next.

The design was worked out in a long back-and-forth with Claude, then built and
taken through six strict phases — infrastructure, state and reward, retrieval and
value, injection, anti-cheating, benchmark — where each phase had to pass its own
acceptance checks before the next one started. That discipline is why the
benchmark's negative result is reported here rather than buried: a phase that
does not prove itself does not advance.
...
<table>
  <tr>
    ...
    <td align="center">
      <a href="https://github.com/claude"><img src="https://github.com/claude.png" width="90" alt="Claude" /></a><br />
      <b>Claude</b><br />
      <sub>AI collaboration partner · architecture review and specification design</sub>
    </td>
  </tr>
</table>

Claude is credited separately from the two human contributors because the role is
different in kind: it did not write the shipped code, it worked on the design —
the phase specifications, the failure modes, and the review that kept the
benchmark honest.
